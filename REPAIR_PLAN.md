...
| A6 | L-17 预警阈值 | 运营预警阈值 remainingHours ≤ 12（凭空设定） | ✅ 已配置化+UI+强制提醒 | 用户口径：**默认 5 课时、可调整**；**剩余 0 课时（最后一节课用完）强制提醒一次，不受阈值影响** → `utils/alert-config.ts`（默认 5，含内存回退）+ 系统设置页「课时不足预警阈值」配置入口 + `statistics.ts` 双规则过滤（≤阈值 或 =0，已用尽单独文案）+ 4 条回归测试 |
