...
| A6 | L-17 预警阈值 | 运营预警阈值 remainingHours ≤ 12（凭空设定） | ✅ 已配置化+UI+强制提醒+待办已读 | 用户口径：**默认 5 课时、可调整**；**剩余 0 课时强制提醒不受阈值影响**；**提醒方式=首页待办事项**（扣课后剩余降到阈值 → 首页「课时续费提醒」待办，**用户手动点已读**，同一轮不重复推送；剩余回升后清除已读可重新提醒）→ `utils/alert-config.ts`（默认5）+ `statistics.ts` 双规则过滤 + `utils/todo-read.ts`（待办已读记录）+ `services/home.ts`（运营预警→学员级待办，过滤已读）+ TodoList「已读」按钮 + 充值/撤销/回补联动 + 9 条回归测试 |
