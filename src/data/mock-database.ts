...
export const CLASSES: Class[] = [
  ...
  // 演示班级（用户口径 2026-08-23）：23:00 未上课演示，保持当天无点名记录
  {
    id: 'cls-demo',
    name: '少儿素描基础班',
    teacherId: 'teacher-004',
  ...
  // 演示班级（用户口径 2026-08-23）：18:00 已下课未点名演示，保持当天无点名记录
  {
    id: 'cls-demo2',
    name: '水彩启蒙班',
    teacherId: 'teacher-004',
  ...
  // 标签演示班（仅班课 Tab）：「上课中」「试听」；团课无试听，勿混用
  {
    id: 'cls-tag-active',
    name: '班课演示·上课中',
    teacherId: 'teacher-004',
  ...
  // 团课标签演示（无试听）：仅「上课中」/ 普通可预约，开放预约
  {
    id: 'cls-group-tag-active',
    name: '团课演示·上课中',
    teacherId: 'teacher-001',
    teachers: ['teacher-001'],
    campusId: 'campus-center',
...
export const STUDENTS: Student[] = [
  ...
  // cls-009 硬笔书法入门 (7人, 周三、周五)
  {
    ...
    campusId: 'campus-west',
    teacherId: 'teacher-004',
    classIds: ['cls-009', 'cls-demo', 'cls-demo2', 'cls-tag-active', 'cls-tag-trial'],
    totalHours: 38,
    remainingHours: 22,
    status: 'active',
    createdAt: '2025-09-15T00:00:00Z',
...
export const SCHEDULES: Schedule[] = [
  ...
  // —— 标签演示：相对当前时刻，今天课表可见「上课中」「试听」——
  ...(() => {
    const hour = new Date().getHours();
    const activeStartH = hour >= 23 ? 21 : hour;
    const activeEndH = Math.min(23, activeStartH + 2);
    const activeStart = `${String(activeStartH).padStart(2, '0')}:00`;
    const activeEnd = `${String(activeEndH).padStart(2, '0')}:59`;
    // 试听班错开上课中时段，便于单独看「试听」标签（若已跨天则仍用错开小时）
    const trialStartH = (activeEndH + 3) % 24;
    const trialEndH = (trialStartH + 1) % 24;
    const trialStart = `${String(trialStartH).padStart(2, '0')}:00`;
    const trialEnd =
      trialEndH > trialStartH
