...
const NotificationSendPage: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const list = await studentStore.fetchByTeacher(currentUserId);
        setStudents(list);
      } catch (err) {
  ...
  }, [selectedIds, students, title, content]);
          ...
          <View className="mb-7">
            ...
            <View className="flex flex-wrap gap-3">
              {UNIQUE_TYPES.map((t) => (
                <View
                  key={t.label}
                  className={`py-4 px-6 rounded-2xl border border-input bg-white flex items-center gap-2 transition ${notifType === t.key && title === '' ? 'border-primary bg-gradient-primary' : ''}`}
                  onClick={() => setNotifType(t.key)}
                >
                  <Text className="text-xl">{t.emoji}</Text>
                  <Text
                    className={`text-base font-medium ${notifType === t.key && title === '' ? 'text-white' : 'text-muted-foreground'}`}
                  >
                    {t.label}
                  </Text>
                </View>
