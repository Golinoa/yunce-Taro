  ...
  const handleBatchCheckin = useCallback(async () => {
    ...
    try {
      for (const stu of presentStudents) {
        ...
        const pkg = packages.find((p) => p.remaining_hours >= 1);
        if (!pkg) {
          Taro.showToast({ title: `${stu.name} 课时不足，已跳过`, icon: 'none' });
          continue;
        }
        ...
        // 扣减课时（悲观更新：以服务端返回为准）
        const updatedPkg = await packageService.deductHours(pkg.id, 1);
        ...
        for (const binding of parents) {
          await notificationService.send({
            ...
            content: `本次核销 1 课时，剩余 ${updatedPkg?.remaining_hours ?? pkg.remaining_hours - 1} 课时`,
            related_id: stu.id,
          });
        }
        successCount++;
    ...
    <PageContainer safeBottom>
      <View className="min-h-screen pb-[200rpx]" style={{ background: '#F5FAF8' }}>
        ...
        <View className="fixed bottom-0 left-0 right-0 py-6 px-8 bg-white/95 backdrop-blur-sm border-t border-input">
          <ActionButton
            text={submitting ? '消课中...' : `批量消课（选中${presentStudents.length}人）`}
