...
import { studentService, classService, scheduleService } from '@/services';
    ...
    const init = async () => {
      try {
        const [stuList, clsList] = await Promise.all([
          ...
          classStore.fetchByTeacher(currentUserId),
        ]);
        setStudents(stuList);
        setClasses(clsList);
        ...
        if (isEdit && scheduleId) {
          ...
          if (sch) {
            setMode(sch.class_id ? 'class' : 'student');
    ...
    <PageContainer safeBottom>
      <View className="min-h-screen bg-gradient-subtle pb-[200rpx]">
        ...
        <View className="fixed bottom-0 left-0 right-0 px-8 py-6 bg-white/95 backdrop-blur-sm border-t-[2rpx] border-input flex gap-4 pb-safe-bar">
          ...
          {isEdit && (
            <View
              className="py-7 px-8 rounded-[24rpx] border-2 border-destructive bg-white flex items-center justify-center"
              onClick={handleDelete}
            >
              <Text className="text-lg text-destructive font-medium">删除排课</Text>
            </View>
          )}
        </View>
      </View>
