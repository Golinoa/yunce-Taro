...
const StudentDetail: React.FC = () => {
  ...
  const [parents, setParents] = useState<StudentParent[]>(null);
  ...
  useShareAppMessage((res) => {
    ...
    return {
      ...
      path: `/pages/parent-bind/index?studentId=${encodeURIComponent(student.id)}&token=${encodeURIComponent(token)}`,
      imageUrl: '', // 使用默认截图
    };
  });
  ...
  const handleDelete = useCallback(() => {
    Taro.showModal({
      title: '确认删除',
      content: '删除后学生数据将无法恢复，是否确认？',
      success: async (res) => {
        if (res.confirm && studentId) {
  ...
  const goToRecordDetail = useCallback((recordId: string) => {
    Taro.navigateTo({ url: `/pages/lesson-detail/index?id=${encodeURIComponent(recordId)}` });
  }, []);

  // 跳转课时充值
  const goToRecharge = useCallback(() => {
    Taro.navigateTo({
      url: `/pages/course-packages/index?studentId=${encodeURIComponent(studentId)}`,
    });
  }, [studentId]);
  ...
  return (
    <View className="min-h-screen bg-background">
        ...
        <View className="flex items-center justify-between relative z-1">
          ...
          {isTeacher && (
            <View
              ...
              onClick={() =>
                Taro.navigateTo({
                  url: `/pages/student-form/index?id=${encodeURIComponent(studentId)}`,
                })
              }
            >
      ...
      <ScrollView
        scrollY
        className="px-[32rpx] pt-[32rpx] pb-[200rpx]"
              ...
              return (
                <View key={pkg.id} className="bg-white rounded-[32rpx] p-[32rpx] shadow-soft">
                  ...
                  <View className="flex gap-[12rpx] mt-[20rpx] flex-wrap">
                    {pkg.fee_amount != null && (
                      <View className="py-[8rpx] px-[20rpx] rounded-[16rpx] bg-[#f5f5f5]">
                        <Text className="text-[24rpx] text-muted-foreground">
                          ¥{pkg.fee_amount}
                        </Text>
                      </View>
          ...
          <View className="flex flex-col gap-[20rpx]">
            ...
            <View className="bg-white rounded-[32rpx] py-[48rpx] px-[48rpx] shadow-soft text-center">
              ...
              <View className="flex gap-[16rpx] mt-[32rpx]">
                ...
                >
                  <Icon name="mdi-content-copy" size={32} color="white" />
                  <Text className="text-white">复制邀请码</Text>
                </View>
                <Button
                  className="flex-1 py-[28rpx] rounded-[40rpx] bg-accent shadow-elegant text-[28rpx] font-medium text-white text-center press-scale flex items-center justify-center gap-[8rpx] border-none after:border-none"
                  openType="share"
                  onClick={handleInviteParent}
                  style={{ lineHeight: 'normal', padding: 0 }}
                >
                  <Icon name="mdi-share-variant" size={32} color="white" />
      ...
      {/* ====== 底部操作栏 ====== */}
      {isTeacher ? (
        <View className="fixed bottom-0 left-0 right-0 bg-white px-[32rpx] pt-[24rpx] pb-safe-bar shadow-soft flex gap-[20rpx] z-10">
          <View
            className="flex-1 h-[96rpx] rounded-[48rpx] bg-gradient-primary center press-scale"
            onClick={goToRecharge}
          >
            <Text className="text-[30rpx] font-semibold text-white">充值</Text>
          </View>
          <View
          ...
          >
            <Text className="text-[30rpx] font-semibold text-primary">编辑</Text>
          </View>
          <View
            className="flex-1 h-[96rpx] rounded-[48rpx] bg-white center press-scale border-[3rpx] border-destructive"
            onClick={handleDelete}
          >
            <Text className="text-[30rpx] font-semibold text-destructive">删除</Text>
          </View>
        </View>
      ) : (
