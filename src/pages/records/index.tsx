...
const RecordsPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (isTeacher) {
        relatedStudents = await studentStore.fetchByTeacher(profile.id);
        for (const s of relatedStudents) {
          const rs = await lessonRecordService.getByStudent(s.id);
          allRecords.push(...rs);
        }
      } else {
  ...
  }, [profile, isTeacher, routeStudentId]);
  ...
  const goDetail = useCallback((id: string) => {
    Taro.navigateTo({ url: `/pages/lesson-detail/index?id=${encodeURIComponent(id)}` });
  }, []);
