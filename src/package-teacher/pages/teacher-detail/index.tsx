...
const TeacherDetailPage: React.FC = () => {
  ...
  }, [teacher, loading, fetchAll]);

  const handleResign = useCallback(
    async (type: ResignType, reason?: string) => {
      if (!teacher || resigning) return;
      setResigning(true);
      try {
        await resignTeacher(teacher.id, type, reason);
        setShowResignSheet(false);
        Taro.showToast({ title: '已标记离职', icon: 'success' });
      } catch {
        Taro.showToast({ title: '标记离职失败', icon: 'none' });
      } finally {
        setResigning(false);
      }
    },
    [teacher, resignTeacher, resigning],
  );

  const handleSalaryAction = useCallback(async () => {
    if (!teacher) return;
    if (teacher.salaryStatus === 'pending') {
      if (confirming) return;
      setConfirming(true);
      try {
        await confirmSalary(teacher.id);
        Taro.showToast({ title: '工资已确认', icon: 'success' });
      } catch {
        Taro.showToast({ title: '工资确认失败', icon: 'none' });
      } finally {
        setConfirming(false);
      }
  ...
  }, [teacher, confirmSalary, confirming]);

  const handlePayConfirm = useCallback(
    async (remark: string) => {
      if (!teacher || paying) return;
      setPaying(true);
      try {
        setPendingPayAction({ type: 'single', ids: [teacher.id] });
        await useTeacherStore.getState().executePay(remark);
    ...
    [teacher, setPendingPayAction, paying],
  );
  ...
  const handleEditSubmit = useCallback(
    async (
      ...
      try {
        ...
        await useTeacherStore.getState().updateTeacher(teacher.id, {
          name: data.name,
          phone: data.phone,
  ...
  return (
    <View className="min-h-screen bg-background flex flex-col">
      ...
      {teacher.status === 'active' && teacher.salaryStatus !== 'paid' && (
        <View className="flex gap-[24rpx] px-[32rpx] py-[24rpx] pb-safe-bar bg-card border-t border-border shrink-0">
          <View
            ...
            onClick={confirming || paying || resigning || editing ? undefined : handleSalaryAction}
          >
      ...
      {teacher.status === 'active' && teacher.salaryStatus === 'paid' && (
        <View className="flex gap-[24rpx] px-[32rpx] py-[24rpx] pb-safe-bar bg-card border-t border-border shrink-0">
          <View
            className={cn('action-btn-danger', (resigning || editing) && 'opacity-60')}
            onClick={resigning || editing ? undefined : () => setShowResignSheet(true)}
          >
            {resigning ? '处理中...' : '标记离职'}
          </View>
        </View>
...
// ===== 反馈Tab =====
const FeedbackTab: React.FC<{ teacher: TeacherUIModel }> = ({ teacher }) => (
  <View className="px-[32rpx] py-[32rpx]">
    <View className="flex flex-col items-center justify-center py-[120rpx]">
      ...
      <Text className="text-[24rpx] text-muted-foreground opacity-70 mt-[8rpx] text-center leading-normal">
        当前联调阶段暂未接入独立教师反馈档案
        {'\n'}
        可先通过意见反馈页补充说明
      </Text>
      <View
        className="mt-[28rpx] px-[28rpx] py-[16rpx] rounded-full bg-primary/10 border border-primary/20 press-scale"
        onClick={() => Taro.navigateTo({ url: '/package-settings/pages/feedback/index' })}
      >
        <Text className="text-[24rpx] text-primary font-medium">去意见反馈</Text>
      </View>
      <Text className="text-[22rpx] text-muted-foreground mt-[16rpx]">
        当前教师：{teacher.name}
      </Text>
    </View>
  </View>
