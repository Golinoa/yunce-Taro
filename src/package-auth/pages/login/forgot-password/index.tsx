...
const ForgotPasswordPage: React.FC = () => {
  ...
  const { sendLabel, sendDisabled, handleSend } = useEmailOtpSend(sendPasswordResetCode);
  ...
  const handleResetPassword = useCallback(async () => {
    ...
    const targetEmail = (resolvedEmail || email).trim().toLowerCase();
    if (!targetEmail) {
      Taro.showToast({ title: '请输入邮箱', icon: 'none' });
      return;
    }
    ...
    Taro.showToast({ title: PASSWORD_RESET_SUCCESS, icon: 'none', duration: 2500 });
    Taro.redirectTo({
      url: `/package-auth/pages/login/index?email=${encodeURIComponent(targetEmail)}&from=reset`,
    });
