 ...
 *
 * ## 根因（务必读完）
 * 1. 微信 `scroll-into-view` 只要仍绑在节点上（含 `""`），任意 setData 都可能回顶 → idle 必须解绑（见 scrollIntoViewProps）。
 * 2. 打开蒙层时的 setState 仍可能让未受控 ScrollView 丢位置 → 打开前用当前 scrollTop **钉住**。
 * 3. 关闭时用 `top → top+0.01 → null` 会**主动驱动滚动条** → 禁止。
 * 4. **pin 释放时直接移除 `scrollTop` prop 同样会驱动滚动条**：Taro 模板恒渲染 `scroll-top="{{p32}}"`，
 *    `removeAttribute` 会把值置为 `''`，微信把 `''` 当 0 → 整页回顶。因此**首次 pin 后 scrollTop 粘性保持**
 *    （`scroll-top` 是一次性命令，不是受控约束，保留旧值不会卡住后续滚动）。
 * 5. 仅靠 onScroll 缓存不可靠（漏事件时 ref=0 → 一点击就钉回顶部）→ freeze 必须用 `scrollOffset` 读真实位置，再在回调里开层。
 * 6. FAB 展开可复用 freeze；与固定蒙层共用同一 pin，勿另起一套。
 *
 * ## 用法
 * ```tsx
 ...
 * // 关弹层：setVisible(false); unfreeze();   // pin 粘性保持，解除不触发回顶
 * ```
 */
...
export function useOverlayScrollFreeze(scrollViewSelector?: string) {
  ...
  /** 首次 pin 后粘性保持的值：解除时不移除 scrollTop，避免 WXML scroll-top="" 触发回顶 */
   ...
   * 禁止：先 setVisible(true) 再 freeze——会在未钉住窗口丢位置。
   */
  ...
  /** 关层后延迟解开；不解绑期间保持同一 scrollTop，禁止 +0.01 微调 */
  ...
  return {
    ...
    unfreeze,
    unfreezeNow,
    /** 粘性保持：首次 pin 后 scrollTop 常驻（scroll-top 为一次性命令），解除/释放绝不写回 '' */
