/**
 * AddNoteSheet - 记笔记弹层
 *
 * 首页 FAB「记笔记」入口；支持设置提醒时间，并入待办时间轴。
 */
import { View, Text, Textarea } from '@tarojs/components';
import Taro from '@tarojs/taro';
import cn from 'classnames';
import dayjs from 'dayjs';
import React, { useCallback, useEffect, useState } from 'react';
import BottomSheet from '@/components/BottomSheet';
import DatePickerSheet from '@/components/DatePickerSheet';
import Icon from '@/components/Icon';
import Switch from '@/components/Switch';
import TimePickerSheet from '@/components/TimePickerSheet';
import type { NoteTagColor } from '@/utils/user-notes';

export interface AddNoteSheetProps {
  visible: boolean;
  onClose: () => void;
  onSubmit: (payload: {
    content: string;
    folder?: string;
    tagColor?: NoteTagColor;
    remindEnabled?: boolean;
    remindDate?: string;
    remindTime?: string;
  }) => Promise<void>;
}

const TAG_OPTIONS: NoteTagColor[] = ['default', 'primary', 'accent', 'warning'];

const AddNoteSheet: React.FC<AddNoteSheetProps> = ({ visible, onClose, onSubmit }) => {
  const [content, setContent] = useState('');
  const [tagColor, setTagColor] = useState<NoteTagColor>('default');
  const [remindEnabled, setRemindEnabled] = useState(false);
  const [remindDate, setRemindDate] = useState(dayjs().format('YYYY-MM-DD'));
  const [remindTime, setRemindTime] = useState(dayjs().add(30, 'minute').format('HH:mm'));
  const [datePickerVisible, setDatePickerVisible] = useState(false);
  const [timePickerVisible, setTimePickerVisible] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const timestamp = dayjs().format('YYYY-MM-DD HH:mm');

  useEffect(() => {
    if (!visible) return;
    setContent('');
    setTagColor('default');
    setRemindEnabled(false);
    setRemindDate(dayjs().format('YYYY-MM-DD'));
    setRemindTime(dayjs().add(30, 'minute').format('HH:mm'));
    setSubmitting(false);
  }, [visible]);

  const handleSubmit = useCallback(async () => {
    const trimmed = content.trim();
    if (!trimmed) {
      Taro.showToast({ title: '请输入笔记内容', icon: 'none' });
      return;
    }
    setSubmitting(true);
    try {
      await onSubmit({
        content: trimmed,
        folder: '收件箱',
        tagColor,
        remindEnabled,
        remindDate: remindEnabled ? remindDate : undefined,
        remindTime: remindEnabled ? remindTime : undefined,
      });
      onClose();
    } catch {
      Taro.showToast({ title: '保存失败，请重试', icon: 'none' });
    } finally {
      setSubmitting(false);
    }
  }, [content, onClose, onSubmit, remindDate, remindEnabled, remindTime, tagColor]);

  return (
    <>
      <BottomSheet
        visible={visible}
        title="记笔记"
        onClose={onClose}
        height="auto"
        maxHeightLimit="78vh"
        scrollable={false}
      >
        <View className="px-[32rpx] pb-[calc(32rpx+env(safe-area-inset-bottom))]">
          <View className="flex flex-row items-center justify-between mb-[20rpx]">
            <View className="flex flex-row items-center gap-[8rpx] rounded-full bg-card border border-border px-[20rpx] py-[10rpx]">
              <Icon name="mdi-inbox" size="xs" color="warning" />
              <Text className="text-[24rpx] text-foreground">收件箱</Text>
            </View>
            <View
              className={cn(
                'rounded-full px-[28rpx] py-[10rpx] press-scale',
                submitting ? 'bg-muted' : 'bg-primary/15',
              )}
              onClick={submitting ? undefined : handleSubmit}
            >
              <Text
                className={cn(
                  'text-[26rpx] font-medium',
                  submitting ? 'text-muted-foreground' : 'text-primary',
                )}
              >
                {submitting ? '保存中...' : '保存'}
              </Text>
            </View>
          </View>

          <View className="rounded-[20rpx] bg-primary/8 border border-primary/15 px-[20rpx] py-[12rpx] mb-[20rpx]">
            <Text className="text-[22rpx] text-primary">📝 {timestamp}</Text>
          </View>

          <View className="rounded-[24rpx] border border-border bg-card px-[24rpx] py-[20rpx] min-h-[280rpx]">
            <Textarea
              className="w-full text-[30rpx] text-foreground min-h-[240rpx]"
              placeholder="好记性不如烂笔头 ~"
              placeholderClass="text-muted-foreground"
              maxlength={2000}
              value={content}
              onInput={(event) => setContent(event.detail.value || '')}
              focus={visible}
            />
          </View>

          <View className="mt-[24rpx] flex flex-row items-center justify-between rounded-[20rpx] border border-border bg-card px-[24rpx] py-[18rpx]">
            <Text className="text-[28rpx] text-foreground">加入待办提醒</Text>
            <Switch checked={remindEnabled} onChange={setRemindEnabled} />
          </View>

          {remindEnabled && (
            <View className="mt-[16rpx] flex flex-row gap-[16rpx]">
              <View
                className="flex-1 rounded-[20rpx] border border-border bg-card px-[20rpx] py-[18rpx] press-scale"
                onClick={() => setDatePickerVisible(true)}
              >
                <Text className="text-[22rpx] text-muted-foreground block">日期</Text>
                <Text className="text-[28rpx] text-foreground mt-[4rpx]">{remindDate}</Text>
              </View>
              <View
                className="flex-1 rounded-[20rpx] border border-border bg-card px-[20rpx] py-[18rpx] press-scale"
                onClick={() => setTimePickerVisible(true)}
              >
                <Text className="text-[22rpx] text-muted-foreground block">时间</Text>
                <Text className="text-[28rpx] text-foreground mt-[4rpx]">{remindTime}</Text>
              </View>
            </View>
          )}

          <View className="mt-[24rpx] flex flex-row gap-[16rpx]">
            {TAG_OPTIONS.map((option) => (
              <View
                key={option}
                className={cn(
                  'flex-1 rounded-[16rpx] border py-[16rpx] center press-scale',
                  tagColor === option ? 'border-primary bg-primary/10' : 'border-border bg-card',
                )}
                onClick={() => setTagColor(option)}
              >
                <Icon
                  name="mdi-checkbox-multiple-marked-outline"
                  size="sm"
                  color={
                    option === 'primary'
                      ? 'primary'
                      : option === 'accent'
                        ? 'accent'
                        : option === 'warning'
                          ? 'warning'
                          : 'muted'
                  }
                />
              </View>
            ))}
          </View>
        </View>
      </BottomSheet>

      <DatePickerSheet
        visible={datePickerVisible}
        value={remindDate}
        onClose={() => setDatePickerVisible(false)}
        onConfirm={(date) => {
          setRemindDate(date);
          setDatePickerVisible(false);
        }}
      />
      <TimePickerSheet
        visible={timePickerVisible}
        value={remindTime}
        onClose={() => setTimePickerVisible(false)}
        onConfirm={(time) => {
          setRemindTime(time);
          setTimePickerVisible(false);
        }}
      />
    </>
  );
};

export default AddNoteSheet;
